...
describe('Theme Override Tests', () => {
  ...
  describe('Base Field Override Tests (Non-Responsive)', () => {
    ...
    it('should override gap base class for non-responsive Stack', () => {
      const overrideFunc = (theme: ThemeProps) => {
        theme.stack.themes.size.gap.base = 'gap-6';
        return theme;
      };
    ...
    it('should override gap base class for non-responsive Row', () => {
      const overrideFunc = (theme: ThemeProps) => {
        theme.row.themes.size.gap.base = 'gap-10';
        return theme;
      };
    ...
    it('should override multiple base fields simultaneously for non-responsive button', () => {
      ...
      const { container } = render(
        <ThemeProvider themeOverride={overrideFunc}>
          <Button>Fully Custom Sizing</Button>
        </ThemeProvider>
      );
    ...
    it('should use base class when not responsive, breakpoint classes when responsive', () => {
      ...
      // Non-responsive should use base
      expect(nonResponsive).toHaveClass('text-base');
      expect(nonResponsive).not.toHaveClass('text-2xl');
      expect(nonResponsive).not.toHaveClass('max-tablet:text-xl');
      expect(nonResponsive).not.toHaveClass('max-mobile:text-lg');

      // Responsive should use breakpoint classes
      expect(responsive).toHaveClass('text-2xl');
      expect(responsive).toHaveClass('max-tablet:text-xl');
      expect(responsive).toHaveClass('max-mobile:text-lg');
      expect(responsive).not.toHaveClass('text-base');
    });
    ...
    it('should override base gap for Card component', () => {
      ...
      expect(card).not.toHaveClass('gap-(--gap)');
    });
    ...
    it('should allow empty string to disable base class output', () => {
      ...
      const { container } = render(
        <ThemeProvider themeOverride={overrideFunc}>
          <Button>No Font Size Class</Button>
        </ThemeProvider>
      );
