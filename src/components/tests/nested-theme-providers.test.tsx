...
describe('Nested ThemeProvider Tests', () => {
  ...
  describe('Theme Override Isolation', () => {
    it('should apply theme overrides only to inner elements', () => {
      // themeOverride callbacks mutate the theme's `defaults` directly.
      ...
      // All titles use CSS consumer class
      expect(outerTitle1).toHaveClass('text-(--text-color)');
      expect(outerTitle2).toHaveClass('text-(--text-color)');
      expect(innerTitle).toHaveClass('text-(--text-color)');

      // Outer titles should have primary appearance from outer override
      expect(outerTitle1).toHaveAttribute('data-appearance', 'primary');
      expect(outerTitle2).toHaveAttribute('data-appearance', 'primary');

      // Inner title should have secondary appearance from inner override
      expect(innerTitle).toHaveAttribute('data-appearance', 'secondary');
    });
  });
  ...
  describe('Complete Isolation Test', () => {
    it('should isolate all theme modifications (overrides, defaults, and extra classes)', () => {
      // themeOverride mutates defaults directly.
      const outerOverride = (theme: ThemeProps) => {
        theme.button.main.defaults = mergeDefaults(
          theme.button.main.defaults as Record<string, boolean>,
          { outline: true }
  ...
  describe('Empty Nested Provider', () => {
    it('should use defaultTheme when nested provider has no customizations', () => {
      ...
      expect(outerTitle).toHaveClass('text-(--text-color)');
      expect(innerTitle).toHaveClass('text-(--text-color)');
    });
  });
});